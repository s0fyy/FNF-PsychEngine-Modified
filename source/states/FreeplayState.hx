...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		addSong('Random', 0, 'face', FlxColor.fromRGB(128, 128, 128));
	...
	override function update(elapsed:Float)
	{
		...
		if (!player.playingMusic)
		{
			...
			if (Paths.formatToSongPath(songs[curSelected].songName) == "random")
				{
		...
		else if (controls.ACCEPT && !player.playingMusic)
		{
			...
			if (songLowercase == "random")
			{
				// trace("Doesn't work yet lol");
				curSelected = FlxG.random.int(2, songs.length - 1);
