...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			// case 'phillyBlazin': new states.stages.PhillyBlazin(); //Week 8
			case 'phillyStreets': new states.stages.PhillyStreets(); //Week 8
		}
		...
		if(ClientPrefs.data.uiType == 'Base Game') timeTxt.visible = false;
		...
		if(ClientPrefs.data.uiType == 'Base Game') timeBar.visible = false;
		...
		if(ClientPrefs.data.uiType == 'Base Game')
			{
				scoreTxt = new FlxText(125, healthBar.y + 40, FlxG.width, "", 20);
		...
		else if(ClientPrefs.data.uiType == 'Psych Engine')
			{
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), 'songs/$songName/chart/'))
			for (file in FileSystem.readDirectory(folder))
			{
				#if LUA_ALLOWED
				if(file.toLowerCase().endsWith('.lua'))
	...
	public function reloadHealthBarColors() {
		...
		if(ClientPrefs.data.uiType == 'Psych Engine')
			{
				healthBar.setColors(FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]),
				FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]));
			}
		else if(ClientPrefs.data.uiType == 'Base Game')
            {
				healthBar.setColors(FlxColor.fromRGB(255, 0, 0), // Opponent
				FlxColor.fromRGB(22, 255, 38)); // Player
			}
	}
	...
	//You don't have to add a song, just saying. You can just do "startDialogue(DialogueBoxPsych.parseDialogue(Paths.ejson(songName + '/dialogue')))" and it should load dialogue.json
	...
	function cacheCountdown()
	{
		...
		for (asset in introAlts) Paths.image('menus/playstate/' + asset);
	...
	{
		var spr:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menus/playstate/' + image));
		spr.cameras = [camHUD];
	...
	private function generateSong(dataPath:String):Void
	{
		...
		var file:String = Paths.ejson(songName + '/chart/events');
		#if MODS_ALLOWED
		if (FileSystem.exists(Paths.modsJson(songName + '/chart/events')) || FileSystem.exists(file))
		#else
	...
	override public function update(elapsed:Float)
	{
		...
		if(ClientPrefs.data.smoothHealth == true) curHealth = FlxMath.lerp(curHealth, health, .2 / (ClientPrefs.data.framerate / 60));
	...
	private function cachePopUpScore()
	{
		...
		for (rating in ratingsData)
			Paths.image('menus/playstate/' + uiPrefix + rating.image + uiSuffix);
		for (i in 0...10)
			Paths.image('menus/playstate/' + uiPrefix + 'num' + i + uiSuffix);
	}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.loadGraphic(Paths.image('menus/playstate/$uiPrefix' + daRating.image + '$uiSuffix'));
		rating.screenCenter();
		...
		var comboSpr:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menus/playstate/' + uiPrefix + 'combo' + uiSuffix));
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menus/playstate/' + uiPrefix + 'num' + Std.int(i) + uiSuffix));
