	...
	**/
	...
	function addSongUI():Void
	{
		...
		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'Load Events', function()
		{
			...
			var file:String = Paths.songs(songName + '/chart/events');
			#if sys
			if (#if MODS_ALLOWED FileSystem.exists(Paths.modsJson(songName + '/chart/events')) || #end FileSystem.exists(file))
			#else
...
/* 			trace('player$i');
	...
	{
		var datathing:CharacterFile = loadCharacterFile(Reflect.field(_song, 'gfVersion'));
 
		if (_song.notes[curSec].mustHitSection)
		{
			leftIcon.changeIcon(characterData.iconP1);
			rightIcon.changeIcon(characterData.iconP2);
			if (_song.notes[curSec].gfSection) leftIcon.changeIcon(datathing.healthicon);
		}
		else
		{
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		// trace(_song.notes[curSec].mustHitSection);

		var mustHitThingy:Bool;
		mustHitThingy = _song.notes[curSec].mustHitSection;

		var plrSelectedlel:String = "Player";
		// trace('Strum: ' + noteStrum);
		if (noteData == 0 || noteData == 1 || noteData == 2 || noteData == 3) {
			if (mustHitThingy == true) {
				plrSelectedlel = 'Player'; }
			else {
				plrSelectedlel = 'Opponent'; }
			}
		else if (noteData == 4 || noteData == 5 || noteData == 6 || noteData == 7) {
			if (mustHitThingy == true) {
				plrSelectedlel = 'Opponent'; }
			else {
				plrSelectedlel = 'Player'; }
			...
			FlxG.sound.play(Paths.sound('chartingSounds/hitNote' + plrSelectedlel));

			// trace('plrSelectedlel = ' + plrSelectedlel);
	...
	function loadJson(song:String):Void
	{
		...
		// what the fuck is that even suppose to mean?? - Sofy
		try {
